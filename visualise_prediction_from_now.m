start_point = max(1,pre_output_index -100);
...
start_time = find(output_sample_pt(:,3) > event(3));
output_sample_pt = output_sample_pt(start_time:end,:);

figure
plot3(temp(:,1),temp(:,2), temp(:,3),'b*', 'MarkerSize', 2);
hold on
